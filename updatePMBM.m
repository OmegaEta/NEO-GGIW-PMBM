...
for j = 1:J
    if isempty(W1)
        ...
        %Optimization
        [P{j},lik,merge_tables{j}] = ObjectsSO(MBM,j,PPP,W1,gating_matrix_d,gating_matrix_u1,model);
    end
    Nj(j) = length(lik);
...
if (sum(idx_keep)==0&&~isempty(wAssoc))||sum(idx_keep)>5%avoid bug
    temp=sort(wAssoc,'descend');
    idx_keep = wAssoc>temp(min(5,length(temp)));
end
...
for j = 1:J
    if ~isempty(P{j})
        ...
        for i = 1:length(P{j})
            ...
            %update merge_table
            merge_table{idx,1}=merge_tables{j};
    ...
    for j = 1:length(meas_track{i})
        for k = 1:size(meas_track{i}{j},1)
            ...
            if any(meas_track{i}{j}(k,:))
                [Bern,lik] = detectionBernfinal(MBM.track{i}(j).Bern,W(:,meas_track{i}{j}(k,:)),model);
            else
...
if n_tt_upd > n_tt
    for i = 1:n_tt_upd - n_tt
        ...
        if any(in_gate)
            [Bern,lik] = detectionPPPfinal(PPP.w(in_gate),PPP.GGIW(in_gate),W(:,meas_track{n_tt+i}{1}),model);
            Bern.t_birth = time;
            Bern.t_death = time;
            Bern.w_death = 1;
        else
...
[tracks,table,merge_table,wAssoc] = newObjectsSO(tracks,table,merge_table,wAssoc,PPP,W,gating_matrix_u,used_meas_u_not_d,time,model);


if ~isempty(tracks)&&n_tt==0
    merge_table{1,1}=zeros(1,length(tracks));
end

%Remove Bernoulli components with low existence probability
n_tt = length(tracks);
del_element=cell(size(table,1),1);%temp variable for modifying merge_table
for i = 1:n_tt
...
%         x.Bern.GGIW(end).V(1,1)/(x.Bern.GGIW(end).v-6) > 20 | x.Bern.GGIW(end).V(2,2)/(x.Bern.GGIW(end).v-6) > 20, tracks{i});
    ...
    for j = 1:length(idx)
        ...
        temp(temp==idx(j)) = 0;
        table(:,i) = temp;
        del_element{idx(j),1}=[del_element{idx(j),1} i];
    end
end

for i=1:size(table,1)%correct merge_table
    if ~isempty(del_element{i,1})
        merge_table{i,1}(del_element{i,1}(del_element{i,1}<=length(merge_table{i,1})))=[];
    end
end
...
if ~isempty(table)
    [ht,ia,ic] = unique(table,'rows','stable');
...
MBM.track = tracks;
MBM.w = wAssoc;
if exist('merge_table','var')
    MBM.merge_table=merge_table;
end
