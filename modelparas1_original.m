load('IntersectedTarget.mat')
load('IntersectedTargetGroundTruth_original.mat')
...
% E = cell(K,1);
% N = zeros(K,1);
% groundTruth_original = cell(K,1);
% % generate tracks (ground truth)
% for targetnum = 1:length(targetTracks)
%     for k = targetTracks(targetnum).birthTime:targetTracks(targetnum).deathTime
%         targetstate = targetTracks(targetnum).x(1:model_original.motionmodel_original.d,k-targetTracks(targetnum).birthTime+1);
...
% 
% for k = 1:K
%     groundTruth_original{k}.x = X{k};
%     groundTruth_original{k}.X = E{k};
% end
...
%Birth model_original
d = 2;
model_original.birth.w = 0.5*ones(nbirths,1);
model_original.birth.GGIW = repmat(struct('a',100,'b',10,'m',[],'P',diag([4,4,4,4]),'v',22.5,'V',72.5*eye(2)),[nbirths,1]);
for i = 1:nbirths
