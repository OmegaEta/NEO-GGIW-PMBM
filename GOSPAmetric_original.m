function [ Error ] = GOSPAmetric_original(X,Y,c,p)
...
for ix = 1:Nx
    for iy = 1:Ny
        ...
        %extensionerror
        extensionerror=extensionerrorMC(X.x(1:2,ix),X.X(:,:,ix),Y.xshape(1:2,:,iy),Y.Xshape(:,:,:,iy));
...
gospa = (gospa + 0.5*(c^p)*(Nx+Ny-2*absGamma)+ExtensionError+10*(Nx+Ny-2*absGamma))^(1/p);
