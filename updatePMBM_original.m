function [PPP,MBM] = updatePMBM_original(PPP,MBM,W,time,model)
...
for j = 1:J
    if isempty(W1)
        ...
        %Optimization
        [P{j},lik] = ObjectsSO_original(MBM,j,PPP,W1,gating_matrix_d,gating_matrix_u1,model);
    end
    Nj(j) = length(lik);
...
if (sum(idx_keep)==0&&~isempty(wAssoc))||sum(idx_keep)>5%%%avoid all removed (will produce bug)
...
%Append new tracks
[tracks,table,wAssoc] = newObjectsSO_original(tracks,table,wAssoc,PPP,W,gating_matrix_u,used_meas_u_not_d,time,model);
