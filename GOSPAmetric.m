...
if Nx==0 && Ny==0
    ...
    MissedError = 0;
    FalseError = 0;
    Error = [gospa,ExtensionError,LocationError,MissedError,FalseError];
    return
elseif Nx==0
    gospa = (0.5*(c^p)*Ny+10*Ny)^(1/p);
    ExtensionError=10*Ny;
    LocationError = 0;
    MissedError = Ny;
    FalseError = 0;
    Error = [gospa,ExtensionError,LocationError,MissedError,FalseError];
    return
elseif Ny==0
    gospa = (0.5*(c^p)*Nx+10*Nx)^(1/p);
    ExtensionError=10*Nx;
    LocationError = 0;
    MissedError = 0;
    FalseError = Nx;
    Error = [gospa,ExtensionError,LocationError,MissedError,FalseError];
...
%         gwd = GaussianWassersteinDistance(X.x(1:2,ix),X.X(:,:,ix),Y.x(1:2,iy),Y.X(:,:,iy));
        ...
        % gwd of shape
        %no need to use auction algorithms, because there only two
        %possibilities
        gwdshape(1)=GaussianWassersteinDistance(X.xshape(1:2,1,ix)+X.x(1:2,ix),X.Xshape(:,:,1,ix),Y.xshape(1:2,1,iy)+Y.x(1:2,iy),Y.Xshape(:,:,1,iy))+...
            GaussianWassersteinDistance(X.xshape(1:2,2,ix)+X.x(1:2,ix),X.Xshape(:,:,2,ix),Y.xshape(1:2,2,iy)+Y.x(1:2,iy),Y.Xshape(:,:,2,iy));
        gwdshape(2)=GaussianWassersteinDistance(X.xshape(1:2,1,ix)+X.x(1:2,ix),X.Xshape(:,:,1,ix),Y.xshape(1:2,2,iy)+Y.x(1:2,iy),Y.Xshape(:,:,2,iy))+...
            GaussianWassersteinDistance(X.xshape(1:2,2,ix)+X.x(1:2,ix),X.Xshape(:,:,2,ix),Y.xshape(1:2,1,iy)+Y.x(1:2,iy),Y.Xshape(:,:,1,iy));
        %extensionerror
        extensionerror=extensionerrorMC(X.xshape(1:2,1:2,ix)+repmat(X.x(1:2,ix),[1 2]),X.Xshape(:,:,1:2,ix),Y.xshape(1:2,1:2,iy)+repmat(Y.x(1:2,iy),[1 2]),Y.Xshape(:,:,1:2,iy));
        
        % Apply threshold c
        D(ix,iy) = min(gwdshape);
        
        %save extensionerror
        H(ix,iy)=sqrt(extensionerror);
    end
end
...
if Ny<=Nx
    ...
    for iy = 1:Ny
        ...
        if D(Customer2Item(iy),iy) < c
            ...
            % Extension error
            ExtensionError = ExtensionError + H(Customer2Item(iy),iy);
    ...
    for ix = 1:Nx
        ...
        if D(ix,Customer2Item(ix)) < c
            ...
            % Hollowness error
            ExtensionError = ExtensionError + H(ix,Customer2Item(ix));
            % Number of assignments
            absGamma = absGamma + 1;
        end
    end
...
gospa = (1/2*gospa + 0.5*(c^p)*(Nx+Ny-2*absGamma)+ExtensionError+10*(Nx+Ny-2*absGamma))^(1/p);
