function [tracks,table,merge_table,wAssoc] = newObjectsSO(tracks,table,merge_table,wAssoc,PPP,W,gating_matrix,used_meas_u_not_d,k,model)
...
for i = 1:n_tt
    ...
    [Bern,lik] = detectionPPPfinal(PPP.w(in_gate),PPP.GGIW(in_gate),W(:,bestPartition{i}),model);
    Bern.t_birth = k;
    Bern.t_death = k;
    Bern.w_death = 1;
